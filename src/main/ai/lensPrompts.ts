 ...
 * user turn; that duplicated journal text into the transcript and made the
 * payload preview understate what was sent. The decision now arrives the same
 * way every other attachment does, and the lens is only the instruction.
 */

import type { LensKind } from '@shared/ipc-contract'

export const LENS_INSTRUCTIONS: Record<LensKind, string> = {
...
export function lensInstruction(kind: LensKind): string {
  return [
    'The user has asked you to analyse the attached decision through a specific lens.',
    'Stay inside the lens — do not give a generic analysis, and do not summarise the decision back to them; they wrote it.',
    '',
    LENS_INSTRUCTIONS[kind]
  ].join('\n')
}
